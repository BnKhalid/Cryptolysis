...
static class DES_Utils
{
    public static readonly int[] exceptionalShifts = [1, 2, 9, 16];
    ...
    public static long Get64(this Random rnd)
    {
    ...
    public static BigInteger Rotate(this BigInteger HexNumber, int shift, int size)
    {
    ...
    public static BigInteger F(BigInteger right, BigInteger key)
    {
        ...
        for (int i = 0; i < 8; i++)
        {
            var row = ((xor & 0b100000) >> 4) | (xor & 1);
            var col = (xor & 0b011110) >> 1;
            sBoxed |= (BigInteger)sBox[i][(int)row][(int)col] << (4 * i);
            xor >>= 6;
        }

        return sBoxed.Permute(p, 32);
    }
}
